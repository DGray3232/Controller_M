...
/**
  * @brief  Инициализирует структуру P-регулятора с заданными параметрами
  * @param  pid Указатель на структуру PID_Controller
  * @param  Kp Коэффициент пропорциональной составляющей
  * @param  alpha Коэффициент фильтрации выходного сигнала (0.0 - 1.0)
  * @param  scale_factor Масштабирующий коэффициент выходного сигнала
...
void P_Init(P_Controller *p, float Kp_p, float alpha_p, float scale_factor_p) {
    p->Kp_p = Kp_p;
    p->alpha_p = alpha_p;
    p->scale_factor_p = scale_factor_p;
    p->previous_output_p = 0.0;
}
/**
  * @brief  Вычисляет управляющий сигнал P-регулятора
  * @param  pid Указатель на структуру P_Controller
  * @param  error Текущая ошибка регулирования (заданное значение - текущее значение)
  * @param  trim Корректирующее смещение для ошибки регулирования
  * @retval Отфильтрованный и масштабированный управляющий сигнал
  * @note   Функция включает:
  ...
  */
  ...
  /**
  * @brief  Сбрасывает накопленные состояния PID-регулятора (интеграл, производную и предыдущие значения)
  * @param  pid Указатель на структуру PID_Controller
  * @retval None
  */
...
float P_Compute(P_Controller *p, float error_p) {
    // Проверка на NaN / INF
    if (isnan(error_p) || isinf(error_p)) {
        error_p = 0.0;
    }
    // Вычисление управляющего сигнала (только пропорциональная часть)
    float output_p = p->Kp_p * error_p;
    // Ограничение выходного сигнала
    float safe_output_p = output_p;
    if (output_p > INT_MAX) safe_output_p = INT_MAX;
    else if (output_p < -INT_MAX) safe_output_p = -INT_MAX;
    // Фильтрация выходного сигнала
    float filtered_output_p = p->alpha_p * safe_output_p + (1 - p->alpha_p) * p->previous_output_p;
    // Проверка на NaN / INF
    if (isnan(filtered_output_p) || isinf(filtered_output_p)) {
        filtered_output_p = 0.0;
    }
    // Масштабирование выходного сигнала
    filtered_output_p *= p->scale_factor_p;
    p->previous_output_p = filtered_output_p; // сохранение текущего выходного сигнала для следующего шага
    return filtered_output_p; // возвращение отфильтрованного управляющего сигнала
}
...
/**
  * @brief  Обновляет коэффицент П
  * @param  pid Указатель на структуру PID_Controller
  * @param  новый коэффицент п
  * @retval None
  */
void PID_2_Update_PID(PID_2_Controller *pid, float Kp,float Ki,float Kd) {
	pid->Kp = Kp;              // Обновление коэффицента П
	pid->Ki = Ki;              // Обновление коэффицента П
	pid->Kd = Kd;              // Обновление коэффицента П
}
...
/**
  ...
  * @param  pid Указатель на структуру PID_Controller
  * @param  error Текущая ошибка регулирования (заданное значение - текущее значение)
  * @param  error_d Текущая ошибка регулирования (заданное значение - текущее значение) для д компоненты
  * @param  trim Корректирующее смещение для ошибки регулирования
  * @retval Отфильтрованный и масштабированный управляющий сигнал
  * @note   Функция включает:
  *         - ограничение интегральной составляющей
...
float PID_2_Compute(PID_2_Controller *pid, float error, float error_d, float dt) {
	//float dt = CONTROL_LOOP_DT;
    // Проверка на NaN / INF
    if (isnan(error) || isinf(error) || isnan(error_d) || isinf(error_d)) {
        error = 0.0;
        error_d = 0.0; // Обнуляем оба сигнала при ошибке в любом из них
    }
    // интегральная часть
    pid->integral += error * dt;
    if (pid->integral > pid->integral_limit) {
    ...
    // Производная часть с фильтрацией
    float raw_derivative = (error_d - pid->error_previous) / dt;
    float filtered_derivative = pid->alpha_derivative * raw_derivative + (1 - pid->alpha_derivative) * pid->previous_derivative;
    pid->previous_derivative = filtered_derivative;
    pid->error_previous = error_d;
    // Вычисление управляющего сигнала
    float output = pid->Kp * error + pid->Ki * pid->integral + pid->Kd * filtered_derivative;
    float safe_output = output;
    if (output > INT_MAX) safe_output = INT_MAX;
